...
def upload_to_google_drive(output_dir):
    ...
    if not google_creds or not drive_folder:
        print("  ⚠️ Google Drive not configured (GOOGLE_CREDENTIALS or DRIVE_FOLDER_ID missing)")
...
def update_google_dashboard(cache_dir):
    ...
    if not google_creds or not sheet_id:
        print("  ⚠️ Dashboard not configured (GOOGLE_CREDENTIALS or DASHBOARD_SHEET_ID missing)")
